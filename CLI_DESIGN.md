...
- **`--config <path>` / `SQLXL_CONFIG` env var for CI/CD** — user feedback: teams want to supply a pre-baked profiles file rather than running `sqlxl init` interactively on each agent. Two scenarios: (1) a shared file on a fileshare or in source control that all developers and pipelines point at; (2) a CI pipeline that injects connection strings from secrets/variables into a config file at pipeline setup time, then calls `sqlxl insert --profile prod` with no interactive step. Implementation is small — `SqlXlConfig.Load()` checks `SQLXL_CONFIG` env var or `--config` flag before falling back to `~/.sqlxl/config.json`. File format stays identical, only the location is overridable.
